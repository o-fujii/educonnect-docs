# EduConnect 共有URL一覧

チーム共有時に使うURLを整理したドキュメントです。

## まず共有するURL

- トップページ: https://o-fujii.github.io/educonnect-docs/
- 現在の推奨版 Ver2: https://o-fujii.github.io/educonnect-docs/ver2/
- 比較ページ: https://o-fujii.github.io/educonnect-docs/versions.html

## 資料別URL

| 資料 | 位置づけ | URL |
|---|---|---|
| Ver1 | 完成形・フル構想版 | https://o-fujii.github.io/educonnect-docs/ver1/ |
| Ver2 | 超MVP・スモールスタート版 | https://o-fujii.github.io/educonnect-docs/ver2/ |
| 比較ページ | Ver1とVer2の違い | https://o-fujii.github.io/educonnect-docs/versions.html |

## 管理用URL

- GitHubリポジトリ: https://github.com/o-fujii/educonnect-docs
- GitHub Pagesトップ: https://o-fujii.github.io/educonnect-docs/

## 補足

- チーム会議では原則としてVer2をベースに議論します。
- Ver1は最終的に目指す完成形の共有用です。
- Ver0は過去版としてリポジトリ内に残っていますが、通常共有の対象外です。
